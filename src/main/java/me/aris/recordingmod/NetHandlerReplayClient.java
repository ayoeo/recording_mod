...
import me.aris.recordingmod.mixins.NetworkPlayerInfoAccessor;
...
public class NetHandlerReplayClient extends NetHandlerPlayClient {
      ...
      if (packetIn.getAction() == SPacketPlayerListItem.Action.REMOVE_PLAYER) {
        ...
        if (networkplayerinfo != null) {
          switch (packetIn.getAction()) {
            case ADD_PLAYER:
              ((NetworkPlayerInfoAccessor) networkplayerinfo).setGameType(spacketplayerlistitem$addplayerdata.getGameMode());
              ((NetworkPlayerInfoAccessor) networkplayerinfo).setResponseTime(spacketplayerlistitem$addplayerdata.getPing());
              break;
...
//          this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.DECLINED));
          ...
          this.client.addScheduledTask(new Runnable() {
            public void run() {
              NetHandlerReplayClient.this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback() {
                public void confirmClicked(boolean result, int id) {
                  NetHandlerReplayClient.this.client = Minecraft.getMinecraft();
                  ServerData serverdata1 = NetHandlerReplayClient.this.client.getCurrentServerData();
...
//                    NetHandlerReplayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.ACCEPTED));
                    Futures.addCallback(NetHandlerReplayClient.this.client.getResourcePackRepository().downloadResourcePack(s, s1), NetHandlerReplayClient.this.createDownloadCallback());
                  } else {
                    if (serverdata1 != null) {
                      serverdata1.setResourceMode(ServerData.ServerResourceMode.DISABLED);
                    }

//                    NetHandlerReplayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.DECLINED));
                  }

                  ServerList.saveSingleServer(serverdata1);
                  NetHandlerReplayClient.this.client.displayGuiScreen((GuiScreen) null);
                }
...
//        NetHandlerReplayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
      }

      public void onFailure(Throwable p_onFailure_1_) {
//        NetHandlerReplayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.FAILED_DOWNLOAD));
      }
    };
  }
